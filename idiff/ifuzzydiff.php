 ...
 * Modified by zelonght to support Fuzzy comparison (with threshold) instead of comparing identically pixel per pixel
 */
...
for ($x = 0; $x < $sx1; $x++) {
    for ($y = 0; $y < $sy1; $y++) {
        ...
        if (abs($pix1['red']-$pix2['red'])>$threshold 
            || abs($pix1['green']-$pix2['green'])>$threshold
            || abs($pix1['blue']-$pix2['blue'])>$threshold)
        { // different pixel
